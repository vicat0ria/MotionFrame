...
export class VideoController {
  ...
  public getSignedPlaybackUrl = async (
    req: Request,
    res: Response,
    next: NextFunction
  ) => {
    try {
      ...
      logger.debug(`Getting signed URL for video ID: ${id}`);
      ...
      try {
        ...
        if (!video || !video.s3VideoKey) {
          logger.error(`Video or S3 key not found for ID: ${id}`);
          throw new AppError("Video not found", 404);
        }

        logger.debug(`Generating signed URL for S3 key: ${video.s3VideoKey}`);

        // Generate signed URL
        const url = await this.s3Service.getSignedUrl(video.s3VideoKey, 3600); // 1 hour

        res.json({ url });
      } catch (error) {
        logger.error(`Error generating signed URL for ${id}:`, error);
        if (error instanceof AppError) {
          throw error;
        }
        throw new AppError("Error generating video URL", 500);
      }
    } catch (err) {
      next(err);
    }
